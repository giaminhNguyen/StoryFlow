...
# The only keys this script decides. Every other line of an existing .env (STORYFLOW_YTDLP_PYTHON,
# STORYFLOW_LISTER_TIMEOUT, STORYFLOW_INBOX_DIR, comments, hand-made settings) is kept exactly as it is.
MANAGED_KEYS = (
    "STORYFLOW_SUBTITLE_PROVIDER", "STORYFLOW_STORY_RUNNER", "STORYFLOW_CLAUDE_CLI", "STORYFLOW_STORY_TIMEOUT",
    "STORYFLOW_TTS_ENGINE", "STORYFLOW_VIENEU_ROOT", "STORYFLOW_VIENEU_PRECISION", "STORYFLOW_VIENEU_THREADS",
    "STORYFLOW_VIENEU_VOICE",
)
ENV_HEADER = "# Managed keys below were written by scripts/configure_providers.py - safe to edit."
INBOX_KEY = "STORYFLOW_INBOX_DIR"
...
def env_key(line: str) -> str | None:
    """The KEY of a ``KEY=value`` line (None for blanks, comments and anything else)."""
    stripped = line.strip()
...
def merge_env_text(existing: str, values: dict[str, str], managed: tuple = MANAGED_KEYS) -> str:
    """``existing`` .env text with the managed keys replaced by ``values``; everything else is kept verbatim.

    A managed key that ``values`` no longer sets (e.g. the user turned TTS off) is removed, so the file never
    keeps a stale half of a setting. Unknown keys, comments and their order survive."""
    kept = [ln for ln in existing.replace("\r\n", "\n").split("\n")
            if env_key(ln) not in managed and ln.strip() != ENV_HEADER]
    while kept and not kept[-1].strip():
        kept.pop()
    block = [ENV_HEADER] + [f"{k}={v}" for k, v in values.items() if k in managed]
    return "\n".join(kept + ([""] if kept else []) + block) + "\n"


def backup_path(env_file: Path, now: datetime | None = None) -> Path:
    """``.env.bak-YYYYmmddHHMMSS`` next to the .env, never an existing file (older backups are never overwritten)."""
    stamp = (now or datetime.now()).strftime("%Y%m%d%H%M%S")
    candidate = env_file.parent / f"{env_file.name}.bak-{stamp}"
    n = 2
    while candidate.exists():
        candidate = env_file.parent / f"{env_file.name}.bak-{stamp}-{n}"
        n += 1
    return candidate
...
def write_env(env: dict[str, str]) -> None:
    ...
    ENV_FILE.write_text(merge_env_text(existing, env), encoding="utf-8", newline="\n")


def inbox_dir_from(env_text: str) -> Path:
    """The inbox folder configured in .env (STORYFLOW_INBOX_DIR), else runtime/inbox."""
    for line in env_text.splitlines():
        if env_key(line) == INBOX_KEY:
            value = line.split("=", 1)[1].strip().strip("\"'")
            if value:
                return Path(value)
    return ROOT / "runtime" / "inbox"


def ensure_inbox_dir() -> Path | None:
    """Create the folder for subtitle files you provide yourself (fresh installs otherwise have none)."""
    text = ENV_FILE.read_text(encoding="utf-8") if ENV_FILE.exists() else ""
    folder = inbox_dir_from(text)
    try:
        folder.mkdir(parents=True, exist_ok=True)
    except OSError:
        return None
    print(f"  Thư mục inbox (đặt file phụ đề <video_id>.txt/.srt/.vtt vào đây): {folder}")
    return folder
...
def main() -> int:
    ...
    if ENV_FILE.exists() and not YES:
        ...
        if not ask_yes("Cấu hình lại từ đầu? (file cũ sẽ được sao lưu)", default=False):
            ...
            configure_channels({})   # installing yt-dlp writes no .env: existing installs can still get channel links
            ensure_inbox_dir()
            return 0
    env: dict[str, str] = {}
