...
def test_presets_map_to_review_settings():
    ...
    assert fast == ReviewSettings(False, False, 1, "block") and DEFAULT_PRESET == "fast"
    assert balanced == ReviewSettings(True, False, 1, "skip")  # verdict + issues recorded, story untouched, advisory
    assert quality == ReviewSettings(True, True, 2, "block")   # reviewer corrects the story, up to 2 rounds
...
@pytest.mark.parametrize("raw", [
    ...
    {"max_rounds": True}, {"max_rounds": 1.5}, {"max_rounds": "2"}, {"on_failure": "ignore"}, {"on_failure": None},
    {"on_failure": True},
])
...
def test_partial_review_settings_fill_in_defaults():
    assert parse_review_settings({"enabled": True}) == ReviewSettings(True, False, 1, "block")
    assert parse_review_settings({"max_rounds": 3}) == ReviewSettings(False, False, 3, "block")
    assert parse_review_settings({"on_failure": "skip"}).on_failure == "skip"
...
def test_review_from_config_explicit_block_is_overlaid_on_the_preset():
    assert review_from_config({"preset": "quality"}) == ReviewSettings(True, True, 2, "block")
    # only the given keys change: the rest still comes from the preset (it is NOT replaced wholesale)
    assert review_from_config({"preset": "quality", "review": {"enabled": False}}) == ReviewSettings(False, True, 2)
    assert review_from_config({"preset": "quality", "review": {"max_rounds": 3}}) == ReviewSettings(True, True, 3)
    assert review_from_config({"preset": "balanced", "review": {"on_failure": "block"}}).on_failure == "block"
    assert review_from_config({"preset": "nope"}) == ReviewSettings()
    assert review_from_config({"preset": ["quality"]}) == ReviewSettings()               # unhashable: no crash


def test_a_null_review_block_is_ignored_at_runtime_so_the_preset_applies():
    assert review_from_config({"preset": "quality", "review": None}) == ReviewSettings(True, True, 2, "block")
    assert review_from_config({"review": None}) == ReviewSettings()
...
def test_apply_preset_records_what_applies():
    ...
    assert explicit["review"] == {**PRESETS["quality"]["review"], "revise": False}   # explicit keys win, rest = preset
    assert apply_preset({"preset": "quality", "review": {"max_rounds": 3}})["review"]["revise"] is True
    assert apply_preset({"preset": None})["preset"] == "fast"
    assert apply_preset({"review": {}})["review"] == PRESETS["fast"]["review"]


def test_apply_preset_refuses_a_null_review_and_a_revise_that_cannot_run():
    with pytest.raises(PresetError):
        apply_preset({"preset": "quality", "review": None})                            # not silently "no review"
    with pytest.raises(PresetError):
        apply_preset({"review": {"revise": True}})                                     # fast preset: nothing would run
    assert apply_preset({"review": {"enabled": True, "revise": True}})["review"]["revise"] is True
...
def test_create_workflow_records_the_overlaid_review_block(workflows):
    cfg = _config(workflows, workflows.create_workflow(
        "w", config={"preset": "quality", "review": {"max_rounds": 3}}).workflow_id)
    assert cfg["review"] == {"enabled": True, "revise": True, "max_rounds": 3, "on_failure": "block"}


@pytest.mark.parametrize("bad", [{"preset": "turbo"}, {"review": {"bogus": 1}}, {"review": {"max_rounds": 9}},
                                 {"preset": 5}, {"review": None}, {"review": {"revise": True}},
                                 {"review": {"on_failure": "shrug"}}])
